...
void Put_Line(uint x0,uint y0,uint x1,uint y1,uint color )
{
 uint t;
 int xe=0,ye=0,dx,dy,dis;
 int cx,cy;
 uint row,col;
 dx=x1-x0;
 dy=y1-y0;
...
void Put_rectange(unsigned int x0,unsigned int y0,unsigned int x,unsigned int y,unsigned int color)
{
  Put_Line(x0,y0,x,y0,color);
  Put_Line(x,y0,x,y,color);
  Put_Line(x0,y,x,y,color);
  Put_Line(x0,y0,x0,y,color);
}
...
void draw_circle(unsigned int x, unsigned int y,unsigned int r, unsigned int color)
{
	unsigned int i,j;
	float rad;
	for(rad=0;rad<=6.28;rad=rad+0.01)
	{
		i=x-r*sin(rad);
		j=y+r*cos(rad);
		Put_pixel(i,j,color);
